...
def train():
    ...
    if accelerator.is_local_main_process:
        ...
        ema = EMA(diffusion, beta=constants.EMA_DECAY, update_after_step=constants.STEP_START_EMA, update_every=constants.UPDATE_EMA_EVERY).to(device)
    ...
    for epoch in itertools.count(init_epoch+1):
        ...
        if accelerator.is_local_main_process:
            ...
            if epoch != 0 and epoch % constants.VALIDATION_INTERVAL == 0:
                with torch.no_grad():
                    validate(ema, valloader, writer, step, device)
            ...
            if epoch != 0 and epoch % constants.SAMPLE_INTERVAL == 0:
                ...
                for i in range(constants.SAMPLE_NUMBER):
                        image = ema.ema_model.sample(batch_size=1)
                        if i == 0:
